...
public final class TfIdfKeywordWriter {
	...
	private static Stream<List<String>> createNgrams(final List<String> tokenSeq) {
		...
		switch (tokenSeq.size()) {
		case 0: {
			result = Stream.empty();
			break;
		}
		case 1: {
			result = Stream.of(tokenSeq);
			break;
		}
		default: {
			result = IntStream.rangeClosed(1, tokenSeq.size())
					.mapToObj(n -> ListSubsequences.createSubsequenceList(tokenSeq, n)).flatMap(List::stream);
			break;
		}
		}
		return result;
